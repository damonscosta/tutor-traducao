        ...
        // --- Funções de Palavra Única (SPEAK WORD) ---
        
        const wordSystemPrompt = "Você é um dicionário e tradutor de inglês para português. Para a palavra em inglês fornecida, retorne APENAS a tradução mais comum e até duas outras traduções relevantes, juntamente com a classe gramatical (substantivo, verbo, adjetivo, etc.) de cada. Use um formato de lista simples com quebras de linha. Não inclua exemplos ou frases de uso. Retorne APENAS o texto puro com as traduções.";
        
        async function fetchGeminiWordTranslation(word) {
            const userQuery = `Traduza a palavra: "${word}"`;
            const payload = {
                contents: [{ parts: [{ text: userQuery }] }],
                systemInstruction: { parts: [{ text: wordSystemPrompt }] },
            };
            
            // Usamos fetchGeminiContent sem passar um botão, pois o loading já é gerenciado no popup
            const result = await fetchGeminiContent(payload);
            return result.candidates?.[0]?.content?.parts?.[0]?.text || "Nenhuma tradução encontrada.";
        }
        ...
        async function handleWordLookup(event) {
            ...
            if (word) {
                ...
                try {
                    const translationText = await fetchGeminiWordTranslation(word);
